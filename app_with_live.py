...
class TranscriberApp:
    """GUI application for the audio transcriber with live transcription support."""
    ...
    def create_live_tab(self):
        ...
        visualizer_frame = ttk.LabelFrame(self.live_tab, text="Audio Visualization", padding="10")
        visualizer_frame.pack(fill=tk.X, pady=10)
        
        # Create the dual audio visualizer
        self.audio_visualizer = DualAudioVisualizer(visualizer_frame, width=700, height=200)
        self.audio_visualizer.pack(fill=tk.X, expand=True, padx=5, pady=5)
    ...
    def start_recording(self):
        ...
        try:
            ...
            if self.live_audio_capture is None:
                # Define callbacks for audio data
                def system_callback(data):
                    self.audio_visualizer.update_system_data(data)
                
                def mic_callback(data):
                    self.audio_visualizer.update_mic_data(data)
                
                def merged_callback(data):
                    if self.live_transcriber:
                        self.live_transcriber.add_audio_data(data)
                
                # Create live audio capture instance
                config = {"sample_rate": 16000, "channels": 1}  # Optimized for speech recognition
                self.live_audio_capture = LiveAudioCapture(
                    config=config,
            ...
            if self.live_transcriber is None:
                ...
                # Create live transcriber instance
    ...
    def stop_recording(self):
        ...
        try:
            ...
            if messagebox.askyesno("Save Audio", "Do you want to save the recorded audio?"):
    ...
    def _save_recorded_audio(self):
        ...
        filename = filedialog.asksaveasfilename(filetypes=filetypes, defaultextension=".wav")
        if filename:
            # Save the audio
            try:
                output_path = self.live_audio_capture.save_merged_audio(filename)
                if output_path:
    ...
    def _update_live_transcription(self, text):
        ...
        # Append the new transcription to the text area
